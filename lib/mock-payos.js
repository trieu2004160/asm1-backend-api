...
class MockPayOS {
  ...
  async createPaymentLink(orderData) {
    try {
      ...
      console.log('🎭 Mock PayOS: Order data received:', JSON.stringify(orderData, null, 2));
      ...
      // Ensure orderCode is a string for URL
      const orderId = String(mockOrderCode);
      const mockPaymentUrl = `${frontendUrl}/mock-payment/${orderId}?amount=${orderData.amount}`;
      
      console.log('✅ Mock PayOS: Payment link created:', mockPaymentUrl);
      console.log('✅ Mock PayOS: Order ID used:', orderId);
      ...
      console.error('❌ Mock PayOS error stack:', error.stack);
      throw error;
    }
  }
